...
```{r}
show_top_roc <- function(df, extraction_date, .top_n = 10) {
  if (!is.numeric(.top_n)) {
    stop("You must provide an integer for the top 'n' constituents.")
  }
  
  # if (.top_n > nrow(extract_constituents(df, extraction_date))) {
  #   stop("You've chosen a greater top 'n' number than there are constituents. Please lower the number.")
  # }
...
```{r}
extract_top_roc <- function(df, extraction_date, .top_n = 10) {
...
```{r}
calculate_weights <- function(df, .date, .top_n = 10) {
...
```{r}
calculate_weights(consolidated_nested, ymd("2019-12-31"), .top_n = 20) %>% 
  print(n = 20)
```

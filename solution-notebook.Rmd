...
### 3.6 Strategy Implementation and Output

We are finally ready to implement the aforementioned strategy on each day. We proceed by getting a vector of dates for which we want to implement the strategy (defined by the `start_date` and `end_date` variables), and then loop over that vector to use it on our `calculate_weights()` function.



```{r}
analysis_dates <- consolidated_nested %>% select(date) %>% slice(10) %>% pull(date)

analysis_dates %>% 
  map(.f = ~ calculate_weights(consolidated_nested, .x, .top_n = 10))

...
Secondly, calculate the 200-day ROC. We create a function to indicate the number of days (`ndays` argument) for the ROC calculation. This would be useful if we'd like to explore different days for the ROC.
...
consolidated_nested <- consolidated_nested %>% 
  filter(date %>% between(start_date - 1, end_date)
  )
```


We can now create a function for the following steps:

1. For each day, get the constituents of the S&P 500.
2. Then, rank each of those constituents by 200-day ROC.
3. Choose the top 10 stocks.
4. Get the corresponding weights for each stock.

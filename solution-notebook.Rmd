...
In order to run this code, you need to have installed [R](https://cran.r-project.org/), ideally [RStudio](https://rstudio.com/products/rstudio/download/#download) too. Also, you would need to install the following packages:
...
Indeed this is the case. We can see, for example, on the first row: TSLA is being added and replaced by AIV. On the next row we see the oppositve operation: AIV is being removed and substituted by TSLA.
...
Let's inspect the dates on which there are no missing values for the `historical` action.

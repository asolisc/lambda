...
For this purpose, we create two functions. We also set as an argument the number of stocks that can be picked. In that way, the strategy can be changed to pick the top 'n' stocks.
...
```{r}
show_top_roc <- function(df, extraction_date, .top_n) {
...
```{r}
extract_top_roc <- function(df, extraction_date, .top_n) {
...
```{r}
calculate_weights <- function(df, .date) {
  chosen_tickers <- extract_top10_roc(df, .date)
...
```{r}
calculate_weights(consolidated_nested, ymd("2019-12-31")) %>% 
  print(n = 20)
```

This is the main function needed for the implementation. At each date, the function tells you what are the weights needed on each of the S&P 500 constituents, ranking those by 200-day ROC and equally weighing the top 10.
